...
# Load CSV file
csv_file = r"C:\Users\ayabe\Downloads\type3_landmarks.csv"
...
def preprocess_landmarks(row):
    landmarks = []
    num_landmarks = 8  # Updated to 8 landmarks
...
dataset = tf.data.Dataset.from_generator(data_generator,
                                         output_signature=(
                                             ...
                                             tf.TensorSpec(shape=(16,), dtype=tf.float32)  # 8 landmarks * 2 coordinates
                                         ))
...
# Set up callbacks
checkpoint_callback = tf.keras.callbacks.ModelCheckpoint('pose_resnet50_model_full.h5', 
                                                        save_best_only=True, 
                                                        monitor='val_loss', 
                                                        mode='min', 
                                                        verbose=1)
